...
module.exports = {
  ...
  signUp: function (username) {
    ...
    db.query("insert into Users (Username, Hash, Salt) values (?,'Ginger','Spells')", ['Sample'], function (error, data) {
      if (error) {
        ...
        console.log('Success inputting into Users with', data);
        res.send(201);
      }
    });
  },
  ...
  getWorkouts: function (cb, username = undefined) {
    ...
    if (username) {
      db.query('select Workouts.name, Users_Workouts.Date from Users_Workouts, Workouts where Users_Workouts.id_Workouts = Workouts.id and Users_Workouts.id_Users = (select id from Users where Users.Username = ?)', [username], function (error, results) {
        cb(results);
      });
    } else {
      ...
      db.query('select distinct Workouts.name, Users_Workouts.Date from Users_Workouts, Workouts where Users_Workouts.id_Workouts = Workouts.id and Users_Workouts.id_Users = (select id from Users where Users.Username = ?)', ["Sample"], function (error, results) {
        cb(results);
      });
